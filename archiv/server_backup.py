...
def check_virtual_env():
    ...
    if venv_path.exists():
        ...
        if not current_executable.samefile(expected_python):
            logger.warning(f"Not running in expected venv! Current: {current_executable}, Expected: {expected_python}")
        else:
            logger.info(f"✓ Running in virtual environment: {venv_path}")
    else:
        logger.info("No .venv directory found - running in system Python")
