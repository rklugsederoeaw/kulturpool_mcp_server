...
# -*- coding: utf-8 -*-
"""
Test script for Kulturerbe MCP Server with Virtual Environment Support
...
def verify_virtual_env():
    ...
    if venv_path.exists():
        ...
        if current_executable.samefile(expected_python):
            ...
            print("⚠ WARNING: Not running in expected virtual environment!")
            print(f"  Expected: {expected_python}")
            print(f"  Current:  {current_executable}")
    else:
        print("ℹ No .venv directory found")
    
    print(f"Virtual ENV: {os.environ.get('VIRTUAL_ENV', 'Not set')}")
    print()

# Verify environment before importing server modules
verify_virtual_env()
...
class MCPServerTester:
    ...
    async def test_kulturpool_get_details(self):
        ...
        try:
            ...
            print(f"[OK] Requested: {response_data.get('requested_objects', 0)}")
            print(f"[OK] Successful: {response_data.get('successful_retrievals', 0)}")
            print(f"[OK] Objects returned: {len(response_data.get('objects', []))}")
            print(f"[OK] Response size: {len(response_text.encode('utf-8'))} bytes")
            
            # Check constraints
            if len(response_data.get('objects', [])) <= 3:
                print("[OK] Object limit met (<= 3)")
            else:
                print("[FAIL] Object limit EXCEEDED")
            ...
            print(f"[FAIL] Details test failed: {e}")
    ...
    async def test_api_connectivity(self):
        ...
        try:
            ...
            if 'hits' in response:
                ...
                print(f"[OK] API responded with {len(response.get('hits', []))} results")
                self.test_results.append("api_connectivity: PASS")
            else:
                print("[FAIL] API response format unexpected")
    ...
    async def run_all_tests(self):
        ...
        print("=" * 60)
        print("KULTURERBE MCP SERVER - TEST SUITE (VENV)")
        print("=" * 60)
        ...
        if failed == 0:
            print("\n[SUCCESS] ALL TESTS PASSED - Server ready for deployment!")
            print("Virtual environment setup is working correctly.")
        else:
            print("\n[WARNING] Some tests failed - check implementations above")
